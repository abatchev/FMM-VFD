...
class FMM{public: /// //////////////////////          Start FMM class. All members public.          ////////////////////// ///
    ...
    std::vector<double> u, t, m; std::vector<char> s;                          //Declare slowness vector u, time vector t, state vector s{0=far, 1=near(heap), 2=frozen, 3=boundary}.
    ...
    void Eikonal(int n){
        int na,nb,nc,q; double a,b,c,f4,f3,f2,un,ua,ub,uc,tt;                  //Declare upwind axial neighbor index offsets na/nb/nc, upwind axial times a/aa, b/bb, c/cc, time step 'f', temp holder 'q'.
        ...
        if(t[n+na]>t[n+nb]){q=na;na=nb;nb=q;}                                  //Sort upwind neighbor times so that a<b<c; 'a' corresponds to major axis of propagation, 'c' to minor axis.
        a=t[n+na]; b=t[n+nb]; c=t[n+nc]; ua=u[n+na]; ub=u[n+nb]; uc=u[n+nc];   //Assign sorted axial upwind neighbor times 'a/b/c', next neigbor times 'aa/bb/cc'.
        f4=h*(u[n]+ua+ub+uc)/4; f3=h*(u[n]+ua+ub)/3; f2=h*(u[n]+ua)/2;         //Compute averged u values for each scheme used.
        if(a+f4>c){tt=(a+b+c+sqrt(3*f4*f4-2*(a*a+b*b+c*c-a*b-b*c-c*a)))/3; if(tt<t[n]){t[n]=tt; m[n]=3;}} // if a+f4>c Solve (t-a)^2+(t-b)^2+(t-c)^2=f^2 for t. If t<t[n], set t[n]=t.
        if(a+f3>b){tt=(a+b+sqrt(2*f3*f3-(a-b)*(a-b)))/2; if(tt<t[n]){t[n]=tt; m[n]=2;}}  //If a+f>b: solve (t-a)^2+(t-b)^2=f^2 for t.
        if(a+f2<t[n]){t[n]=a+f2; m[n]=1;}}                                     //Test t=a+f. If t<t[n], set t[n]=t.
    ...
    void Eikonal2(int n){
        ...
        else if(a+f>c){t[n]=fmin(t[n],(a+b+c+sqrt(3*f*f-2*(a*a+b*b+c*c-a*b-b*c-c*a)))/3);} //Else if a+f>c Solve (t-a)^2+(t-b)^2+(t-c)^2=f^2 for t. If t<t[n], set t[n]=t.
        else if(a+f>b){t[n]=fmin(t[n],(a+b+sqrt(2*f*f-(a-b)*(a-b)))/2);}       //Else if a+f>b solve (t-a)^2+(t-b)^2=f^2 for t. If t<t[n], set t[n]=t.
    ...
    void Vidale(int n){
        int a,b,c,q; double f,tt,r;                                                                          //Declare upwind axial neighbor index offsets a,b,c, temp placeholder q, time step 'f'.
        ...
        if(s[n+c]==2 && s[n+b+c]==2 && s[n+c+a]==2 && s[n+a+b]==2 && s[n+a+b+c]==2 && s[n+b]==2 && s[n+a]==2){//If 7 upwind neighbors required in "scheme A" are frozen, apply (eq2)(Vidale, 1990).
            ...
            r=(6*f*f-pow(t[n+a]-t[n+b],2)-pow(t[n+b]-t[n+c],2)-pow(t[n+c]-t[n+a],2)-pow(t[n+a+b]-t[n+b+c],2)-pow(t[n+b+c]-t[n+c+a],2)-pow(t[n+c+a]-t[n+a+b],2))/2;
            if(r>0){tt=t[n+a+b+c]+sqrt(r); if(tt>t[n+c] && tt<t[n]){t[n]=tt; m[n]=7;}}}                      //apply (eq2), set as t[n] if greater than largest neighbor time.
            ...
            r=2*f*f-.5*pow(t[n+a+b+c]-t[n+a+b-c],2)-pow(t[n+a]-t[n+b],2);                                    //Compute root term in  scheme B
            if(r>0){tt=t[n+a+b]+sqrt(r); if(tt>t[n+b]&& tt<t[n]){t[n]=tt; m[n]=6;}}}                         //apply (eq3), set as t[n] if greater than largest neighbor time.
        else if(s[n+a]==2 && s[n+a+b]==2 && s[n+a-b]==2 && s[n+a+c]==2 && s[n+a-c]==2){                      //If 5 upwind neighbors required in "scheme C" are frozen, apply (eq4)(Vidale, 1990).
            f=h*(u[n]+u[n+a]+u[n+a+b]+u[n+a-b]+u[n+a+c]+u[n+a-c])/6;                                         //Compute unit step time f=(h*u_avg)^2 where h=grid spacing, u_av= 6 point slowness average.
            r=f*f-(pow(t[n+a+b]-t[n+a-b],2)+pow(t[n+a+c]-t[n+a-c],2))/4;                                     //Compute root term in  scheme C
            if(r>0){tt=t[n+a]+sqrt(r); if(tt<t[n]){t[n]=tt; m[n]=5;}}}}                                      //apply (eq4), set as t[n] if greater than largest neighbor time.
    ...
    void Init(double x0, double y0, double z0, double r0){
        ...
        for(int n=0; n<N; n++){ int i=n%di, j=(n/di)%dj, k=n/(di*dj);          //Iterate through every index to seed 't', 's', 'heap' within 'r0' of source with initial values. Compute axial indices i,j,k at n.
            ...
            if(i==0||i==di-1 || j==0||j==dj-1 || k==0||k==dk-1){s[n]=3;}       //Deactivate all grid boundary nodes setting their s=3 to prevent their addition to 'heap'.
                ...
                t[n]=(u[n]+u[n0])*r/2; s[n]=2; if(r>r0-h){s[n]=1; add(n);}}}}  //Find line travel time from source to n using mean u, set s[n]=2(frozen). If n is near initialization shell and not on boundary, set s=1(active), add to heap.
    ...
    FMM(int _di, int _dj, int _dk, double _h, std::vector<double>_u){
        ...
        m.assign(N,0);                                                         ///INITIALIZE SCHEME ARRAY M FOR DEMO PURPOSES///
